...
func GetScheduledTime(annotations map[string]string) (*time.Time, error) {
	timeRaw := annotations[skprmetav1.ScheduledAnnotation]
	if len(timeRaw) == 0 {
		return nil, nil
	}
...
func GetNextSchedule(scheduledSpec skprmetav1.ScheduledSpec, scheduledStatus skprmetav1.ScheduledStatus, creationTime, now time.Time) (time.Time, time.Time, error) {
	...
	if scheduledStatus.LastScheduleTime != nil {
		earliestTime = scheduledStatus.LastScheduleTime.Time
	} else {
