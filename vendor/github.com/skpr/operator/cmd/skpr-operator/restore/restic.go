...
import (
	...
	resticrestore "github.com/skpr/operator/internal/controller/backup/restore"
)
...
func (d *ResticCommand) run(c *kingpin.ParseContext) error {
	...
	if err := resticrestore.Add(mgr, d.params); err != nil {
...
func Restic(app *kingpin.CmdClause) {
	c := &ResticCommand{}
	cmd := app.Command("restic", "Start the Restic Restore operator").Action(c.run)
	...
	cmd.Flag("cpu", "CPU which will be assigned to a Pod executing the restore").
		Envar("SKPR_BACKUP_RESTIC_CPU").
		Default("100m").
		StringVar(&c.params.Pod.CPU)
	cmd.Flag("memory", "Memory which will be assigned to a Pod executing the restore").
	...
	cmd.Flag("workingdir", "Directory where restore steps will be executed").
