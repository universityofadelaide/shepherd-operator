...
func (d *SESCommand) run(c *kingpin.ParseContext) error {
	...
	var (
		...
		sesClient = ses.New(session.New(&aws.Config{Region: aws.String(d.params.Region)}))
	)
...
func SES(app *kingpin.CmdClause) {
	...
	cmd.Flag("prefix", "Prefix to used for reference when creating AWS resources").
		Envar("SKPR_OPERATOR_SES_PREFIX").
		Required().
		StringVar(&c.params.Prefix)
	cmd.Flag("region", "Region of the AWS SES endpoint").
		Envar("SKPR_OPERATOR_SES_REGION").
		Default("us-east-1").
		StringVar(&c.params.Region)
	cmd.Flag("hostname", "Hostname of the AWS SES endpoint").
		Envar("SKPR_OPERATOR_SES_HOSTNAME").
		Default("email-smtp.us-east-1.amazonaws.com").
		StringVar(&c.params.Hostname)
	cmd.Flag("port", "Prefix to used for reference when creating AWS resources").
		Envar("SKPR_OPERATOR_SES_PORT").
		Default("1025").
